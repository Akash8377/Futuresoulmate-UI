...
const TodaysMatches = ({chatBoxOpen}) => {
  ...
  const handleBlock = async (blockedUserId) => {
    try {
      ...
      // Optionally remove blocked profile from matches
      setProfiles(prev => prev.filter(profile => profile.user_id !== blockedUserId));
        ...
        <div className="row g-0">
          <div className="col-md-4">
            ...
            <div className="verified-profile mt-3 p-3 bg-light rounded">
              {(currentProfile.facebook_connected === 1 || currentProfile.linkedin_connected === 1) && (
                <h4 className="h6">
              ...
              <div className="sub-partverified">
                <ul className="list-unstyled mb-0 small">
                  ...
                  <li className="mb-1">
                    {currentProfile.linkedin_connected === 1 ? (
                      <>
                  ...
                  <li className="mb-1">
                    {currentProfile.facebook_connected === 1 ? (
                      <>
                        <i className="fa fa-check text-success me-2" aria-hidden="true"></i>
          ...
          <div className="col-md-8">
            ...
            <div className="profile-newmatch">
              <div className="row g-0">
                ...
                <div className="col-md-3 d-flex align-items-center justify-content-center connect-now p-2">
                  {/* Disable connect options if user is blocked */}
                  {isBlocked ? (
                    <div className="text-center">
                      <div className="text-danger mb-2">
                        <i className="fa fa-ban fa-2x"></i>
                      </div>
                      <p className="small text-muted">User is blocked</p>
                      <button 
                        className="btn btn-sm btn-outline-primary"
                        onClick={() => handleUnblock(currentProfile.user_id)}
                      >
                        Unblock
                      </button>
                    </div>
            ...
            {/* Hide profile details if user is blocked */}
            {!isBlocked ? (
              <div className="match-tab">
                ...
                <div className="tab-content pt-3">
                  {activeTab === "detailed" && <ProfileDetails {...{ currentProfile }} />}
                  {activeTab === "partner" && <PartnerPreferences {...{ currentProfile }} user={user} />}
                </div>
              </div>
            ) : (
              <div className="alert alert-warning mt-3">
                <i className="fa fa-exclamation-triangle me-2"></i>
                This user is blocked. Profile details are hidden.
              </div>
            )}
          </div>
        </div>
